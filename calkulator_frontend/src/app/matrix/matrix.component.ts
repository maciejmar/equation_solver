...
// });
 ...
 ordinatesForm: FormGroup = this.formbOrdinates.group({
  ...
  initializeForms() {
    ...
    // Listen to changes on the degree and update matrix/ordinates forms accordingly
    // const degreeValue = this.profileForm.get('degree')?.value;
    // if (degreeValue !== undefined) {
       this.profileForm.get('degree')?.valueChanges.subscribe(degree => {
       this.updateMatrixAndOrdinatesForms(degree);
    });
    ...
    updateMatrixAndOrdinatesForms(degree: number) {
      ...
      onSubmit1(): void {
        ...
        const degree = this.profileForm.value.degree;
        // You may want to call your API or perform some action here
      }
      ...
      submitMatrixForm(): void {
        if (this.matrixForm.valid) {
          ...
          // Call your API or perform some action here
        } else {
      ...
      submitOrdinatesForm(): void {
        if (this.ordinatesForm.valid) {
          // Handle valid form submission
          const ordinatesData = this.ordinatesForm.value;
          console.log(ordinatesData);
          // Call your API or perform some action here
        } else {
          console.error('Invalid ordinates form');
          // Optionally, mark all fields as touched to show validation errors
          this.ordinatesForm.markAllAsTouched();
        }
      }
